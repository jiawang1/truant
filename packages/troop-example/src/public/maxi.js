 ...
 * @license MIT http://troopjs.mit-license.org/ © Mikael Karon mailto:mikael@karon.se
 */
define('troopjs-data/store/component',[ "troopjs-core/component/gadget", "when", "poly/array" ], function StoreModule(Gadget, when) {
	"use strict";
...
define('troopjs-data/query/service', [
	...
	"when",
	"troopjs-core/logger/pubsub"
], function QueryServiceModule(module, Service, Query, merge, when, logger) {
	"use strict";
	...
	var LENGTH = "length";
	var BATCHES = "batches";
	var SCAN_TIMEOUT_ID = "scanTimeoutId";
	var SCAN_FIRST_REQUEST_TIME = "scanFirstRequestTime";
	var SCAN_LAST_REQUEST_TIME = "scanLastRequestTime";
	var CACHE = "cache";
	var QUERIES = "queries";
	var RESOLVED = "resolved";
	var CONFIGURATION = "configuration";
	...
	function scan() {
		...
		if ((now - me[SCAN_FIRST_REQUEST_TIME]) < FORCE_SCAN_DURATION &&
			(now - me[SCAN_LAST_REQUEST_TIME]) < SCAN_TIMEOUT) {
			// Renew timer to collect more queries
			me[SCAN_TIMEOUT_ID] = setTimeout(scan.bind(me), SCAN_TIMEOUT);
			return;
		}

		me[SCAN_TIMEOUT_ID] = 0;
		...
		function request() {
			...
			for (i = batches[LENGTH]; i--;) {
				// Add batch[Q] to q
				ARRAY_PUSH.apply(qlist, batches[i][Q]);
			}
			var q = qlist.join("|");
			...
			}, me[CONFIGURATION])).otherwise(function (xhr) {
				var endDate = new Date().getTime();
				if (!xhr.status === 0 || (endDate - startDate) > 1000) {
					throw xhr;
				}
				return me.publish("ajax", merge.call({
					"data": {
					...
					type: me[CONFIGURATION].type,
					url: me[CONFIGURATION].url + (me[CONFIGURATION].url.indexOf('?') >= 0 ? '&' : '?') + 'retry'
				})).then(function (result) {
					var retryEndDate = new Date().getTime();
					logger.log('Retry ajax query success, duration=' + (retryEndDate - endDate));
					return result;
				}, function (error) {
					var retryEndDate = new Date().getTime();
					logger.log('Retry ajax query error, duration=' + (retryEndDate - endDate));
					throw error;
				});
			});
		}
	...
	function requestScan() {
		var me = this;
		var now = new Date().getTime();
		me[SCAN_LAST_REQUEST_TIME] = now;
		if (!me[SCAN_TIMEOUT_ID]) {
			me[SCAN_FIRST_REQUEST_TIME] = now;
			me[SCAN_TIMEOUT_ID] = setTimeout(scan.bind(me), SCAN_TIMEOUT);
		}
	}
	...
	return Service.extend(function QueryService(cache) {
		...
		"sig/stop" : function stop() {
			...
			if (me[SCAN_TIMEOUT_ID]) {
				clearTimeout(me[SCAN_TIMEOUT_ID]);
				me[SCAN_TIMEOUT_ID] = 0;
			}
		},
 ...
 * @license MIT http://troopjs.mit-license.org/ © Mikael Karon mailto:mikael@karon.se
 */
define('troopjs-core/pubsub/proxy/to1x',[ "../../component/service", "when", "poly/array", "poly/object" ], function To1xModule(Service, when) {
	"use strict";
	...
	function isDeferredLike(value) {
		return Boolean(value) && typeof value.resolve === 'function';
	}
				...
				me[SETTINGS].forEach(function (setting) {
					...
					OBJECT_KEYS(subscribe).forEach(function (source) {
						...
						var callback = subscribe[source] = function () {
							...
							if (isDeferredLike(args[args[LENGTH] - 1])) {
								deferred = args.pop();
							}
							...
							if (deferred) {
								when(result, function (array) {
									return deferred.resolve.apply(this, array);
